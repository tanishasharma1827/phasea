...
def run_ocr(image_path, min_conf_for_regex=0.5):
    ...
    QUICK FIX (2026-09-08): full_text is now built ONLY from regions with
    confidence >= min_conf_for_regex. Confirmed real bug: regex previously
    ran against ALL regions concatenated regardless of position or
    confidence, so an unrelated low-confidence digit anywhere on the image
    could get matched immediately after "MRP" just because of string order,
    not because the two were actually adjacent on the label (real example:
    barcode 8906010500702, 'MRP' at bbox x[52,73] and a stray '0' at
    bbox x[132,150] -- ~70px apart, confidence 0.389 -- matched as "MRP 0").
    `regions` still returns ALL detections (including low-confidence ones)
    for the YOLO/bbox pipeline, which already applies its own 0.30 threshold
    separately -- only the regex-matching text is affected by this filter."""
    ...
    for text, score, box in zip(texts, scores, boxes):
        ...
        if float(score) >= min_conf_for_regex:
            parts.append(text)
        ...
        for idx, r in enumerate(kept):
            ...
            for field_type, pattern in FIELD_PATTERNS.items():
                for m in pattern.finditer(full_text):
                    ...
                    suspicious = False
                    if field_type == "MRP":
                        # QUICK FIX (2026-09-08): a bare MRP value under Rs.10 is rare for
                        # real packaged commodities and far more likely to be OCR noise/a
                        # stray digit than a genuine price -- confirmed on a real example
                        # (barcode 8906010500702: matched "MRP 0" from two spatially
                        # unrelated OCR regions). Don't silently drop it (that would hide
                        # a real, if low-confidence, signal) -- flag it instead, at a
                        # visibly lower confidence, so it sorts to the bottom of any
                        # human review queue rather than looking as trustworthy as a
                        # well-formed "MRP 301".
                        digits = re.search(r"\d+(?:[.,]\d+)?", matched_text)
                        if digits:
                            try:
                    ...
                    decl_f.write(json.dumps({
                        "image_id": r["image_id"], "product_id": r["barcode"],
                        "field_type": field_type, "text": matched_text, "bbox": None,
                        "source": "OpenFoodFacts", "annotation_method": "AUTO_REGEX_ON_OCR",
                        "confidence": confidence,
                        "suspicious_low_value": suspicious,
                    }) + "\n")
                    decl_count += 1
